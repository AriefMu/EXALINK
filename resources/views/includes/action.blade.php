...
<div class="modal fade" id="tolakModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="{{ url('pinjam/tolak/'.$row->id) }}" method="POST">
          @method('GET')
          @csrf
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Tolak Pengajuan</h1>
        <button type="button" class="btn-close " id="closeTolakmBtn"data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
          <input type="hidden" name="pinjam_tolak_id" id="pinjam_tolak_id">
          <label class="font-weight-bold">ALASAN</label> 
       <textarea class="form-control" id="alasan" name="alasan" rows="4" cols="50"></textarea>
      </div>
      <div class="modal-footer">
...
{{-- END --}}
{{-- Konfirmasi Setuju Modal --}}
{{-- <div class="modal fade" id="tolakModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="{{ url('pinjam/setuju/'.$row->id) }}" method="POST">
          @method('GET')
          @csrf
      <div class="modal-header">
    ...
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body" id="modalbody">
          <form id="editForm" action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input  class="form-control @error('nama_kegiatan') is-invalid @enderror" value="{{ old('nama_kegiatan',$row->namakeg) }}" name="namakeg" placeholder="Masukkan Nama Kegiatan">                                                           
            </div>
            <div class="form-group">
              <label class="font-weight-bold">NAMA RUANG</label>
              <div class="row">
                <div class="col-12">
              <select  data-dropdown-parent="#edit{{$row->id}}"  class="livesearch5 form-control @error('ruang_id') is-invalid @enderror "  name="ruang_id" style="width:100%">
                 ...
                 <div class="form-group">
                   <label for="mulai" class="font-weight-bold">MULAI</label>
                    <div class='input-group' >
                       <input type='datetime-local' id="mulai" class="form-control" name="mulai"  value="{{ old('mulai', $row->mulai ) }}"/>
       ...
       <div class="form-group">
           ...
           <input type="number" class="form-control @error('durasi') is-invalid @enderror" name="durasi" rows="5" placeholder="Masukkan Durasi" min="1" max="24" value="{{ old('durasi',round((strtotime($row->selesai) - strtotime($row->mulai))/3600))}}"/>
...
<div class="row">
          <div class="col">
               <select  data-dropdown-parent="#edit{{$row->id}}"  class="livesearch6 form-control @error('penanggung_jawab') is-invalid @enderror"  name="penanggung_jawab" style="width: 100%">
  ...
  <div class="row">
            <div class="col">
              <textarea class="form-control" id="alasan" name="alasan" rows="4" cols="20">{{$row->alasan}}</textarea>
            </div>
          </div>
        ...
        <div class="modal-footer">
          ...
          <div class="col">
            ...
            <button type="submit" class="btn btn-primary" formaction="{{ route('pinjam.edit',$row->id) }}" >Simpan</button>
          </div>
          <div class="col">
           
          </div>
          <div class="col">
            <button type="button" data-bs-dismiss="modal" class="btn btn-danger col  deletePinjamBtn" value="{{$row->id}}">Hapus</button>
           ...
           <div class="col-5 mt-1 ">
              <div class="btn-group " role="group" aria-label="Basic example" style="height: 100%">
                @method('GET')
                <button type="submit" class="btn btn-sm btn-success" formaction="{{ url('pinjam/setuju/'.$row->id) }}" >Setuju</button>
            {{-- <a href="{{ url('pinjam/setuju/'.$row->id) }}" class="btn btn-sm btn-success ">SETUJU</a>     --}}
          </form>
            <button type="button"  data-bs-dismiss="modal" class="btn btn-sm btn-warning tolakPinjamBtn" value="{{$row->id}}">TOLAK</button>
              </div>
           </div>
...
function askForSave() {
    form.action="{{ route('pinjam.update',$row->id) }}";
    form.submit();
}
